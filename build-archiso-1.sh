...
if ! command -v yay &>/dev/null && ! command -v paru &>/dev/null; then
    ...
    # makepkg MUST NOT run as root — get the real invoking user
    ...
    # Pre-install all yay build deps as root before dropping to user
    # go, git, base-devel must be present BEFORE makepkg runs or it
    # will interactively prompt even with --noconfirm
    sudo pacman -S --needed --noconfirm go git base-devel gcc || \
        error "Cannot install yay build deps"

    # Use /tmp to avoid $HOME ownership issues (especially if invoked via sudo -i)
    TMP_YAY=$(mktemp -d /tmp/yay-build-XXXXXX)
    chown "$BUILD_USER:$BUILD_USER" "$TMP_YAY"

    # --depth=1 — no need for full git history
    sudo -u "$BUILD_USER" git clone --depth=1 \
        https://aur.archlinux.org/yay.git "$TMP_YAY/yay" || \
        error "Failed to clone yay from AUR"

    # Set GOPATH/GOMODCACHE inside tmp so go module cache is predictable
    # and does not bleed into the users real HOME cache
    ( cd "$TMP_YAY/yay" && \
      GOPATH="$TMP_YAY/go" \
      GOMODCACHE="$TMP_YAY/go/pkg/mod" \
      sudo -u "$BUILD_USER" makepkg -si --noconfirm --needed ) || \
        error "yay makepkg failed — check build output above"

    rm -rf "$TMP_YAY"
    command -v yay &>/dev/null || error "yay installed but binary not found in PATH"
    success "yay installed: $(yay --version)"
fi
AUR_CMD=$(command -v yay || command -v paru)
[[ -z "$AUR_CMD" ]] && error "No AUR helper found after install attempt"
...
base
base-devel
# linux removed — only linux-lts is used for boot, having both wastes ~35MB ISO space
linux-lts
linux-lts-headers
linux-firmware
# linux-headers removed — only needed for DKMS, not at runtime
mkinitcpio
mkinitcpio-archiso
go
sudo
...
ttf-liberation
ttf-dejavu
# ── Calamares + ALL required runtime backend tools (Qt6 era, 2025) ───────────
# Source: Artix Linux calamares 3.3.14 PKGBUILD + calamares-git AUR deps
calamares
# KDE Frameworks required by Calamares modules
kconfig
kcoreaddons
kcrash
kirigami
...
python-pyaml
python-toml
# System utilities used by Calamares modules
hwinfo
ckbcomp
libpwquality
gtk-update-icon-cache
# Filesystem tools (used by partition + unpackfs modules)
squashfs-tools
dosfstools
e2fsprogs
exfatprogs
f2fs-tools
cryptsetup
# polkit Qt6 binding (required for privilege escalation)
polkit-qt6
# NOTE: boost-libs is a BUILD dep only (makedepends), not runtime — do not add here
# ── Audio ─────────────────────────────────────────────────────────────────────
pipewire
pipewire-alsa
pipewire-pulse
...
dolphin
ark
# syslinux removed — profiledef.sh uses UEFI-only bootmodes, syslinux unused
EOF
...
# Wait for full plasma shell, not just graphical.target
# plasma-plasmashell.service ensures the desktop is actually ready
...
Type=oneshot
User=1000
# Do not hardcode DISPLAY/WAYLAND_DISPLAY — inherit from PAM session env
# Hardcoded values cause the service to fail if the compositor chose different values
...
// QtQuick 2.15 required for Calamares Qt6 build (calamares 3.3+)
import QtQuick 2.15;
...
# 30GB storage minimum — Calamares install + kernel/initramfs on EFI + Steam cache
storageMinSize: 30000
# 4GB RAM minimum for KDE Plasma + Steam (8GB recommended)
ramMinSize:      4096
EOF
...
efi:
  ...
  minimumSize:     300MiB
  # Kernel + initramfs must fit on EFI partition since Limine uses boot():/
  # 512MiB recommended, 300MiB minimum (lts kernel=16M + initramfs=184M)
...
# Enable LVM and LUKS support in the partition module
...
unpack:
  - source: "/"
    ...
    exclude:
      ...
      - "mnt"
      - "lost+found"
      # Exclude live-ISO-specific boot configs — installed system gets its own
      # via shellprocess@post. These contain archisobasedir/archisolabel cmdline
      # args that would break the installed system if copied over.
...
cat > "$AIROOTFS/etc/calamares/modules/hostname.conf" << 'EOF'
---
location: /etc/hostname
transitionMethod: auto
EOF
...
# nvme and nvme_core MUST be explicit — autodetect misses them when
# the initramfs is built outside the target machine (e.g. via Calamares chroot)
# Without these the system drops to rootfs shell with "device did not show up"
modules:
  - nvme
  - nvme_core
  - nvme_fabrics
...
---
# 1800s (30min) — yay build + mkinitcpio + kernel copy can exceed 600s on slow connections
dontChroot: false
timeout:    1800
script:
  - "-": |
      grep -q '^\[multilib\]' /etc/pacman.conf || \
          printf '\n[multilib]\nInclude = /etc/pacman.d/mirrorlist\n' >> /etc/pacman.conf
      pacman -Sy --noconfirm
      # Generate locales — localecfg writes locale.gen but does not run locale-gen
      locale-gen 2>/dev/null || true
      # Set locale in /etc/locale.conf if not already set
  ...
  - "-": |
      ...
      # Pre-install ALL yay build deps as root BEFORE dropping to user.
      # makepkg -si inside a Calamares chroot cannot sudo to install deps
      # because sudo/polkit is not configured at shellprocess time.
      # Pre-installing avoids the interactive dep-install prompt entirely.
      pacman -S --needed --noconfirm git base-devel go gcc binutils fakeroot || {
          echo "WARNING: Could not install yay build deps — skipping yay install"
          exit 0
      }

      # Use /tmp — user HOME may not be fully mounted/chowned yet
      YAY_BUILD=$(mktemp -d /tmp/yay-build-XXXXXX)
      chown "$INSTALLED_USER:$INSTALLED_USER" "$YAY_BUILD"

      sudo -u "$INSTALLED_USER" git clone --depth=1 \
          https://aur.archlinux.org/yay.git "$YAY_BUILD/yay" || {
          echo "WARNING: Failed to clone yay — skipping"
          rm -rf "$YAY_BUILD"; exit 0
      }
      chown -R "$INSTALLED_USER:$INSTALLED_USER" "$YAY_BUILD"

      # Use -s (sync deps) not -si (sync+install) — we install the package
      # manually with pacman -U to avoid sudo requirement inside makepkg
      ( cd "$YAY_BUILD/yay" && \
        GOPATH="$YAY_BUILD/go" \
        GOMODCACHE="$YAY_BUILD/go/pkg/mod" \
        BUILDDIR="$YAY_BUILD/build" \
        sudo -u "$INSTALLED_USER" makepkg -s --noconfirm --needed ) || {
          echo "WARNING: yay makepkg failed — skipping"
          rm -rf "$YAY_BUILD"; exit 0
      }

      # Install the built package as root with pacman -U
      YAY_PKG=$(find "$YAY_BUILD/yay" -name "yay-*.pkg.tar.zst" 2>/dev/null | head -1)
      if [ -n "$YAY_PKG" ]; then
          pacman -U --noconfirm "$YAY_PKG" && echo "yay installed OK" || \
              echo "WARNING: pacman -U yay failed"
      else
          echo "WARNING: yay package not found after build"
      fi
      rm -rf "$YAY_BUILD"

  - "-": |
      systemctl enable NetworkManager sddm bluetooth fstrim.timer
      # Enable user-level services that need to be active at login
      systemctl enable --global gamemoded 2>/dev/null || true
      # Add installed user to gamemode group
      # Required for gamemoded to accept optimization requests
      INSTALLED_USER=$(getent passwd 1000 | cut -d: -f1 || echo "user")
      usermod -aG gamemode "$INSTALLED_USER" 2>/dev/null || true
      # Add to input group for gamescope input handling
      usermod -aG input "$INSTALLED_USER" 2>/dev/null || true
      echo "Added $INSTALLED_USER to gamemode and input groups"
...
SDDMEOF
  ...
  - "-": |
      # Write a complete mkinitcpio.conf with NVMe modules explicitly included.
      # CRITICAL: Without nvme/nvme_core the system drops to rootfs shell on boot
      # with "device did not show up after 30 seconds" because autodetect
      # misses NVMe when the initramfs is built inside a Calamares chroot.
      cat > /etc/mkinitcpio.conf << 'MKEOF'
# Generated by SteamOS build-archiso.sh
# nvme + nvme_core are explicit to guarantee NVMe boot on all systems
MODULES=(nvme nvme_core nvme_fabrics)
BINARIES=()
FILES=()
...
MKEOF
      ...
      # Rebuild initramfs with the new config
      mkinitcpio -P 2>&1 || echo "WARNING: mkinitcpio -P failed — check manually"

  - "-": |
      # arch-deckify-setup.service is already deployed via unpackfs from airootfs
      # (with correct graphical-session.target and EnvironmentFile settings)
      # Just enable it — do NOT rewrite it here with stale hardcoded DISPLAY vars
      systemctl enable arch-deckify-setup 2>/dev/null || true
  ...
  # ── Limine post-install: deploy EFI binary + write limine.conf ──────────────
  # Runs on the INSTALLED system after Calamares unpacks and mounts everything.
  # Handles everything Calamares bootloader module may miss for Limine v10.
  - "-": |
      set -e
      EFI_DIR="/boot/efi"
      # Detect root partition UUID dynamically — never hardcode device names
      # findmnt gives us the actual root device regardless of drive name
      ROOT_PART=\$(findmnt -n -o SOURCE / 2>/dev/null || echo "")
      ROOT_UUID=\$(blkid -s UUID -o value "\$ROOT_PART" 2>/dev/null || true)
      [ -z "\$ROOT_UUID" ] && echo "ERROR: Cannot determine root UUID" && exit 1
      KERNEL_BASE="vmlinuz-linux-lts"
      INITRAMFS_BASE="initramfs-linux-lts.img"
      FALLBACK_BASE="initramfs-linux-lts-fallback.img"

      # Find Limine EFI binary
      LIMINE_EFI=""
      for f in /usr/share/limine/BOOTX64.EFI /usr/lib/limine/BOOTX64.EFI; do
          [ -f "\$f" ] && LIMINE_EFI="\$f" && break
      done
      [ -z "\$LIMINE_EFI" ] && echo "ERROR: Limine EFI binary not found" && exit 1

      # Deploy EFI binary
      mkdir -p "\$EFI_DIR/EFI/BOOT" "\$EFI_DIR/EFI/limine"
      cp "\$LIMINE_EFI" "\$EFI_DIR/EFI/BOOT/BOOTX64.EFI"
      cp "\$LIMINE_EFI" "\$EFI_DIR/EFI/limine/BOOTX64.EFI"

      # Copy BIOS support files
      for f in limine-bios.sys limine-bios-cd.bin; do
          for d in /usr/share/limine /usr/lib/limine; do
              [ -f "\$d/\$f" ] && cp "\$d/\$f" "\$EFI_DIR/EFI/limine/" && break || true
          done
      done

      # Write limine.conf to all 4 locations Limine v10 searches
      for dest in "\$EFI_DIR/EFI/BOOT/limine.conf" "\$EFI_DIR/EFI/limine/limine.conf" "/boot/limine/limine.conf" "/boot/limine.conf"; do
...
/SteamOS Arch Linux
    protocol: linux
    path: boot():/\$KERNEL_BASE
    cmdline: root=UUID=\$ROOT_UUID rw quiet splash plymouth.enable=1
    module_path: boot():/\$INITRAMFS_BASE

/SteamOS Arch Linux (fallback)
    protocol: linux
    path: boot():/\$KERNEL_BASE
    cmdline: root=UUID=\$ROOT_UUID rw
    module_path: boot():/\$FALLBACK_BASE
LCEOF
      done

      # Remove any stale .cfg files that would override .conf
      rm -f "\$EFI_DIR/EFI/BOOT/limine.cfg" "\$EFI_DIR/EFI/limine/limine.cfg" /boot/limine.cfg 2>/dev/null || true

  # Fix gamescope-session-plus D-Bus issue on installed system
  # Same fix as in customize_airootfs.sh — must be applied post-install too
  # Source: bazzite issue #3282 (Oct 2025)
  - "-": |
      GSSP="/usr/share/wayland-sessions/gamescope-session-plus.desktop"
      [ -f "\$GSSP" ] && \
          sed -i 's|^Exec=.*gamescope-session-plus.*|Exec=/usr/bin/dbus-run-session /usr/bin/gamescope-session-plus steam|' "\$GSSP" && \
          echo "Patched gamescope-session-plus.desktop on installed system" || true
      rm -f /usr/share/wayland-sessions/gamescope-session.desktop 2>/dev/null || true
      # Also update sddm.conf to use correct session name
      INSTALLED_USER=\$(getent passwd 1000 | cut -d: -f1 || echo "user")
      sed -i "s/^Session=gamescope-session-steam/Session=gamescope-session-plus/" /etc/sddm.conf 2>/dev/null || true
      # Ensure gamescope env config exists for installed user
      USER_HOME=\$(getent passwd 1000 | cut -d: -f6 || echo "/home/\$INSTALLED_USER")
      mkdir -p "\$USER_HOME/.config/environment.d"
      cat > "\$USER_HOME/.config/environment.d/gamescope-session-plus.conf" << GSCEOF
CLIENTCMD="steam -steamos3 -steampal -steamdeck -gamepadui -pipewire-dmabuf"
GAMESCOPECMD="gamescope -e --xwayland-count 2 --default-touch-mode 4 --hide-cursor-delay 3000 --fade-out-duration 200"
GSCEOF
      chown -R "\$INSTALLED_USER:\$INSTALLED_USER" "\$USER_HOME/.config" 2>/dev/null || true
      echo "gamescope-session-plus environment configured for \$INSTALLED_USER"

      # Register UEFI NVRAM entry — detect actual EFI partition dynamically
      # Do not hardcode nvme0n1 — installer may target any drive
      EFI_PART=\$(findmnt -n -o SOURCE /boot/efi 2>/dev/null || echo "")
      if [ -n "\$EFI_PART" ]; then
          EFI_DISK=\$(lsblk -ndo pkname "\$EFI_PART" 2>/dev/null || echo "")
          EFI_PARTNUM=\$(lsblk -ndo MAJ:MIN "\$EFI_PART" | cut -d: -f2 | tr -d '[:space:]')
          # Get partition number properly
          EFI_PARTNUM=\$(cat /sys/class/block/\$(basename \$EFI_PART)/partition 2>/dev/null || echo "1")
          [ -n "\$EFI_DISK" ] && efibootmgr --create \
              --disk "/dev/\$EFI_DISK" --part "\$EFI_PARTNUM" \
              --label "SteamOS" \
              --loader "\\\\EFI\\\\BOOT\\\\BOOTX64.EFI" \
              --unicode 2>/dev/null && echo "UEFI entry created for /dev/\$EFI_DISK" || \
              echo "WARNING: efibootmgr failed — may need manual UEFI entry"
      else
          echo "WARNING: Cannot detect EFI partition for NVRAM entry"
      fi

      # Install pacman hook so Limine redeploys automatically on updates
      mkdir -p /etc/pacman.d/hooks
      cat > /etc/pacman.d/hooks/99-limine.hook << HOOKEOF
[Trigger]
Operation = Install
Operation = Upgrade
Type = Package
...
[Action]
Description = Redeploying Limine EFI binary after upgrade...
When = PostTransaction
# Redeploy EFI binary to both BOOT fallback and limine dirs
# Source: Arch Linux Limine wiki + limine USAGE.md v10.x
Exec = /bin/sh -c 'cp /usr/share/limine/BOOTX64.EFI /boot/efi/EFI/BOOT/BOOTX64.EFI && cp /usr/share/limine/BOOTX64.EFI /boot/efi/EFI/limine/BOOTX64.EFI && echo Limine EFI redeployed OK'
HOOKEOF

      # Pacman hook — sync kernel and initramfs to EFI partition on every update
      # Required because Limine uses boot():/ which reads from the EFI partition
      cat > /etc/pacman.d/hooks/98-sync-kernel-efi.hook << 'KERNHOOKEOF'
[Trigger]
Operation = Install
Operation = Upgrade
Type = Package
...
[Action]
Description = Syncing kernel and initramfs to EFI partition...
When = PostTransaction
Exec = /bin/sh -c 'cp /boot/vmlinuz-linux-lts /boot/efi/vmlinuz-linux-lts && cp /boot/initramfs-linux-lts.img /boot/efi/initramfs-linux-lts.img && cp /boot/initramfs-linux-lts-fallback.img /boot/efi/initramfs-linux-lts-fallback.img 2>/dev/null; echo Kernel synced to EFI partition'
KERNHOOKEOF

      # Copy kernel and initramfs to EFI partition
      # Limine config uses boot():/ which resolves to the EFI partition
      # so both files must live there, not just in /boot
      cp /boot/vmlinuz-linux-lts "\$EFI_DIR/vmlinuz-linux-lts" 2>/dev/null || true
      cp /boot/initramfs-linux-lts.img "\$EFI_DIR/initramfs-linux-lts.img" 2>/dev/null || true
      cp /boot/initramfs-linux-lts-fallback.img "\$EFI_DIR/initramfs-linux-lts-fallback.img" 2>/dev/null || true
      echo "Kernel and initramfs copied to EFI partition"
      ls -lh "\$EFI_DIR/vmlinuz-linux-lts" "\$EFI_DIR/initramfs-linux-lts.img" 2>/dev/null || true
      echo "Limine post-install complete. Root UUID: \$ROOT_UUID"
EOF
...
---
# CRITICAL: Calamares has NO native Limine module (2025). "limine" is invalid.
# Use "none" to skip; Limine deployed by shellprocess@post instead.
efiBootLoader:      "none"
kernel:             "/boot/vmlinuz-linux-lts"
initramfs:          "/boot/initramfs-linux-lts.img"
initramfsBackup:    "/boot/initramfs-linux-lts-fallback.img"
...
Type=Application
Categories=System;
# X-KDE-autostart-phase=2 ensures it starts after desktop shell is ready
X-KDE-autostart-phase=2
# OnlyShowIn prevents it from auto-launching on the installed system
OnlyShowIn=KDE;
# NoDisplay=false so it appears in app menus on the live session
NoDisplay=false
EOF
...
/$PRODUCT_NAME
    protocol: linux
    path: boot():/arch/boot/x86_64/vmlinuz-linux
...
/$PRODUCT_NAME (verbose)
    ...
    module_path: boot():/arch/boot/x86_64/initramfs-linux.img
EOF
cp /usr/share/limine/limine-bios.sys "$AIROOTFS/boot/limine/" 2>/dev/null || true
rm -f "$AIROOTFS/boot/limine/limine.cfg" 2>/dev/null || true
info "Written: limine.conf v10 + limine-bios.sys staged + stale .cfg removed"
...
# Fix 1: Remove duplicate gamescope-session.desktop (AUR bug, 2025)
# gamescope-session-steam-git installs a stale symlink that creates a
# duplicate "Gamescope" entry in SDDM.
# Source: AUR gamescope-session-steam-git comments + PKGBUILD diff (2025)
rm -f /usr/share/wayland-sessions/gamescope-session.desktop 2>/dev/null || true
echo "  Removed duplicate gamescope-session.desktop"

# Fix 2: Patch gamescope-session-plus.desktop to wrap with dbus-run-session
# ROOT CAUSE of black screen on first login: SDDM launches gamescope-session-plus
# outside a user D-Bus scope. dbus-update-activation-environment fails with
# "unable to connect to D-Bus" causing an immediate session exit loop.
# FIX: Wrap Exec with dbus-run-session to provide a session bus.
# Source: bazzite issue #3282 (Oct 2025) confirmed fix
GSSP_DESKTOP="/usr/share/wayland-sessions/gamescope-session-plus.desktop"
if [ -f "$GSSP_DESKTOP" ]; then
    sed -i 's|^Exec=.*gamescope-session-plus.*steam.*|Exec=/usr/bin/dbus-run-session /usr/bin/gamescope-session-plus steam|' "$GSSP_DESKTOP"
    echo "  Patched gamescope-session-plus.desktop with dbus-run-session"
else
    # Create it from scratch if missing
    cat > /usr/share/wayland-sessions/gamescope-session-plus.desktop << 'GSPEOF'
[Desktop Entry]
Name=Gaming Mode
...
GSPEOF
    echo "  Created gamescope-session-plus.desktop with dbus-run-session"
fi
systemctl enable arch-deckify-setup || true

# Fix 3: Pre-create gamescope-session-plus environment config in /etc/skel
# gamescope-session-plus sources ~/.config/environment.d/*.conf for config.
# Without it, gamescope gets no resolution hints and may show a black screen.
# Source: ChimeraOS/gamescope-session README + AUR comments 2025
mkdir -p /etc/skel/.config/environment.d
cat > /etc/skel/.config/environment.d/gamescope-session-plus.conf << 'GSCEOF'
# Gamescope session environment — auto-generated by build-archiso.sh
# Gamescope will auto-detect resolution if SCREEN_WIDTH/HEIGHT are not set.
# Uncomment and set these if you get a black screen or wrong resolution:
#SCREEN_WIDTH=1920
#SCREEN_HEIGHT=1080
#INTERNAL_WIDTH=1280
#INTERNAL_HEIGHT=800
# Enable adaptive sync (VRR) if your display supports it
#ADAPTIVE_SYNC=1
# Enable HDR if supported
#ENABLE_GAMESCOPE_HDR=0
# Steam Big Picture mode command — this is the correct default for Steam Deck UI
# Source: ChimeraOS gamescope-session-steam default CLIENTCMD
CLIENTCMD="steam -steamos3 -steampal -steamdeck -gamepadui -pipewire-dmabuf"
# Gamescope flags — -e = embedded/fullscreen mode required for Steam session
GAMESCOPECMD="gamescope -e --xwayland-count 2 --default-touch-mode 4 --hide-cursor-delay 3000 --fade-out-duration 200"
GSCEOF
echo "  Created /etc/skel gamescope-session-plus.conf"
...
if ! id liveuser &>/dev/null; then
    # gamemode group required for gamemoded to accept requests
    # Source: FeralInteractive/gamemode README
    useradd -M -G wheel,video,audio,storage,optical,gamemode -s /bin/bash liveuser
fi
# Also add to gamemode group if user already exists
usermod -aG gamemode liveuser 2>/dev/null || true
...
if [ "\$1" == "plasma" ] || [ "\$1" == "desktop" ]; then
    ...
    # Session name must match the .desktop filename in /usr/share/wayland-sessions/
    # gamescope-session-plus.desktop -> session name is "gamescope-session-plus"
    # Source: AUR gamescope-session-git README + SDDM session lookup behavior
    NEW_SESSION="gamescope-session-plus"
    sudo sed -i "s/^Session=.*/Session=\${NEW_SESSION}/" "\$CONFIG_FILE"
